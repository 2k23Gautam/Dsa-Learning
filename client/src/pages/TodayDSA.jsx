...
function TodayDSASkeleton() {
  return (
    <div className="space-y-6 animate-pulse">
      {[...Array(2)].map((_, i) => (
        <div key={i} className="space-y-3">
          <div className="flex items-center gap-2">
            <div className="h-5 bg-slate-200 dark:bg-white/[0.04] rounded w-32" />
            <div className="h-4 bg-slate-200 dark:bg-white/[0.04] rounded-full w-8" />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
            {[...Array(3)].map((_, j) => (
              <div key={j} className="gradient-glass p-4 border border-slate-200/50 dark:border-white/[0.06] rounded-2xl h-28" />
            ))}
          </div>
        </div>
      ))}
...
export default function TodayDSA() {
  const { problems, todayStr, problemsLoading } = useStore();
  ...
  const openAdd  = () => { setDrawerState({ open: true, problem: null, initialTab: 'edit' }); };
  const openEdit = (p) => { setDrawerState({ open: true, problem: p, initialTab: 'overview' }); };
...
function Section({ title, icon, count, empty, items, onEdit, setDrawerState, accentColor, glowColor }) {
  return (
    <div>
